...
Status: validated in conversation 2026-09-02. Do not implement until explicit go.
...
- Tone: cyber HUD (cyan / emerald / USDT) on a luxury dark-gold ground. Not Freepik mint-purple, not red gambling UI.
...
- Real brand marks: TrueMoney Wallet, PromptPay, and Thai bank logos for SCB BBL BAY KBANK KTB TTB CIMB LHBANK UOB KKP GSB BAAC GHB ISBT TISCO TCRB. Stored in-app.
- New feature: scan PromptPay / merchant QR → fill proxy → confirm amount → `transferQRPromptpay`. Never fire on scan.
- Approach A: restyle tokens + add QR on the existing PromptPay tab. Do not rebuild routes or the TMNOne SDK.
...
| Mix HUD + luxury | Pure HUD, Gatsby-only, casino red | Matches Crown Tether + clip + tuxedo ref |
...
| Approach A restyle | Cinematic home split, character-centric rebuild | Lowest risk to live TMNOne flow |
| Confirm after scan | Instant transfer | Operator desk, large tickets |
| In-app logo files | Hotlink bank CDNs | Offline + no IP on transfer path |
| Easing table below | Springs, bounce, loops | Luxury enter, snappy exit |
...
## Implementation order (when given go)

1. Tokens + keyframes in `src/styles.css`
2. Real logos under `public/brands/`
3. PromptPay scan control on transfer tab
4. Success/sync overlay using the supplied clip stills
5. Bank picker + history row marks
6. Verify on ~393px and production deploy
